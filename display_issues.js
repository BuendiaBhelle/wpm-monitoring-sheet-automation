...
async function displaySitesToBeReported() {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if ((security_score === score[0] || security_score === score[1] || security_score === score[2] || security_score === score[3]) && (site !== "CFHEC" && site !== "EPS" && site !== "GPS" && site !== "CDG" && site !== "RLX" && site !== "IN")) {
    ...
    for (let j = 0; j < data.length; j++) {
        ...
        if ((first_byte_time === score[0] || first_byte_time === score[1] || first_byte_time === score[2] || first_byte_time === score[3]) && (site !== "CFHEC" && site !== "EPS" && site !== "GPS" && site !== "CDG" && site !== "RLX" && site !== "IN")) {
            console.log("* " + site + " - " + screenshot_link)
        }
    }
    console.log("-----------------------")
